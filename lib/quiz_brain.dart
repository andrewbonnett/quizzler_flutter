...
class QuizBrain {
  ...
  int _numQuestionsCorrect = 0;

  //the underscore makes this List PRIVATE, so that the data cannot be manipulated in main.dart.
  ...
  bool checkAnswer(bool userPickedAnswer) {
    if (userPickedAnswer == _questionList[_questionNumber].questionAnswer) {
  ...
  bool isFinished() {
    if (_questionNumber >= _questionList.length - 1) {
      print('isFinished returning true');
      return true;
    } else {
      return false;
    }
